...
// Request logging middleware to log API requests
app.use((req, res, next) => {
  const originalSend = res.send;
  res.send = function (body) {
    if (req.url.startsWith("/api")) {
      const logLine = `[${new Date().toISOString()}] ${req.method} ${req.url} - Status: ${res.statusCode} - IP: ${req.ip}\n`;
      fs.appendFileSync(path.join(process.cwd(), "api-requests.log"), logLine, "utf8");
    }
...
const playlistJobs = new Map<string, PlaylistJob>();
...
async function scrapeYoutubeTitle(url: string): Promise<string> {
  try {
...
async function callCobalt(videoUrl: string): Promise<string> {
  const endpoints = [
